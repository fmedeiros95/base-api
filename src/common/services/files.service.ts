...
export class FilesService {
	...
	private storagePaths: any;

	constructor(
		private readonly awsS3Service: AwsS3Service,
		private readonly configService: ConfigService,
	) {
		this.storagePaths = this.configService.get('storagePaths');
	}
	...
	async loadFile(key: string) {
		if (this.configService.get('STORAGE_TYPE') !== 's3') {
			...
			const buffer = readFileSync(join(this.storagePaths.uploads, key));
	...
	uploadFile(file: Express.Multer.File, path?: string) {
		if (this.configService.get('STORAGE_TYPE') !== 's3') {
			...
			const saveFile =
				(path ? `${path}/${uuidv4()}` : uuidv4()) + `.${extension}`;
			const filepath = join(this.storagePaths.uploads, saveFile);
	...
	deleteFile(key: string) {
		if (this.configService.get('STORAGE_TYPE') !== 's3') {
			const filepath = join(this.storagePaths.uploads, key);
			try {
				unlinkSync(filepath);
			} catch (error) {
